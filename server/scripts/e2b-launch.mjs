...
try{
 sandbox=await Sandbox.create(request.templateID,{apiKey:process.env.E2B_API_KEY,timeoutMs:request.timeout*1000,metadata:request.metadata,allowInternetAccess:true});
 await sandbox.commands.run('/usr/local/bin/botinc-runtime run',{background:true,envs:request.envVars,timeoutMs:0});
