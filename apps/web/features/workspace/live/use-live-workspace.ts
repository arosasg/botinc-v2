  ...
  void (async()=>{
   ...
   const hydrate=async(options:{reuseInventory?:boolean}={})=>{
    ...
    try{
     ...
     patch.suggested14=[];patch.repos14=repos.repositories.map(r=>({id:r.id,name:r.full_name,full:r.full_name,branch:r.default_branch,status:"Connected",state:"Connected",tone:"ok14",provider:"GitHub",connected:true,meta:`Default branch ${r.default_branch}`,environmentVariableKeys:r.environment_variable_keys||[],environmentSecretKeys:r.environment_secret_keys||[]}));
...
export function installActions(logic:Logic,ws:WorkspaceClient,api:Client,me:User,people:PeopleIndex,hydrate:(options?:{reuseInventory?:boolean})=>Promise<void>,fail:(err:unknown)=>void){
 ...
 bind("renderVals",()=>{
  ...
  if(selectedRepository){const environmentCount=(selectedRepository.environmentVariableKeys||[]).length;const secretCount=(selectedRepository.environmentSecretKeys||[]).length;v.repoName14=selectedRepository.full;v.repoMeta14=`Default branch ${selectedRepository.branch}`;v.repoState14=hasGitHubConnection?"Connected":"Needs reconnection";v.repoTone14=hasGitHubConnection?"ok14":"bad14";v.repoConnected14=hasGitHubConnection;v.repoHealth14=[{title:"GitHub connection",detail:hasGitHubConnection?"Available to this workspace":"Reconnect GitHub before starting repository work",tone:hasGitHubConnection?"ok14":"bad14",icon:"/i15.svg#"+(hasGitHubConnection?"circle-check":"circle-alert"),when:""},{title:"Default branch",detail:selectedRepository.branch,tone:"ok14",icon:"/i15.svg#git-branch",when:""},{title:"Repository environment",detail:environmentCount?`${environmentCount} variables stored, ${secretCount} ${secretCount===1?"secret":"secrets"}`:"No variables stored",tone:environmentCount?"ok14":"warn14",icon:"/i15.svg#"+(environmentCount?"circle-check":"circle-alert"),when:""}];v.repoConfigNote14=environmentCount?"This repository's encrypted environment materializes as an ignored .env file when Operator checks it out.":"No repository-scoped environment variables are stored for this repository.";}
